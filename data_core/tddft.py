...
def import_TDDFT(file_name, width, x_window, resolution = 1):
    energies = []
    strengths = []
    ...
    x = np.array([i for i in range(x_window[0], x_window[1], resolution)])

    datas = []

    for band in bands:
        datas.append(Data(x, band.eval(x)))
...
for i in range(1, 10):
    l.append(import_TDDFT("Test_data/test data/out_TDDFT_Ni4_SCH38_DCM_50.log", i, (200, 1000)))
